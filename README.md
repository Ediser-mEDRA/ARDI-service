...
=================
   ...
   * [ARDITO registration tools](#ardito-registration-tools)
      * [Registration via the API for B2B registrations](#registration-via-the-api-for-b2b-registrations)
        ...
        * [DRS submission with wrong attribute value of an identifier](#drs-submission-with-wrong-attribute-value-of-an-identifier)
        * [DRS submission with wrong element](#drs-submission-with-wrong-element)
...
## DRS submission with correct parameters
 ...
 ================================ 
  ...
  The response body provided through the REST API in the form of a JSON includes:
   ...
   |Statement subfields   | Description  |
   | :------------------  | :------------|
   |ardi|An alphanumeric string representing a default value of the Identifier.|
   |subtype|A type of a ProprietaryIdentifier. This element is only permitted if IdentifierType='ProprietaryIdentifier'|
   |asserterName|A Name of the Party|
   |asserterDateTime|The date/time that the Assertion of the information in the DRS is made by the Asserter. By default this will be the date/time that the DRS is created or issued, but it could refer to an earlier time. The DRS is assumed to remain true thereafter unless a replacement DRS or deletion DRS is issued in format number|
   |hubKey|Entity ID embedded into a URL to create unique browsable URL|
   |drs|The whole original DRS submitted in XML format. Noticed that, if the original DRS in XML format doesn't contains the ARDI identifier after the submission, in case of **SUCCESS**, the field will contain the ARDI identifier generated by the web application|
   
   | Accounting subfields | Description  |
   | :--------         | :----------
   | ardi              | The ARDI identifier provided during the submission or the ARDI generated by the web application|
  ...
  + Response 400 Bad Request (application/json; charset=UTF-8):
    ...
    * Body
               
               {
                ...
                "status": 400,
	              "timestamp": "05-07-2018 14:21:32",
	              "message": "org.xml.sax.SAXParseException; lineNumber: 5; columnNumber: 22; cvc-complex-type.2.4.a: Invalid content was found starting with element 'Rights'. One of '{\"http://www.rightscom.com/2011/drs#\":Right}' is expected.",
	              "path": "/statement",
	              "error": "Bad Request"
   ...
   ## Authorization
 The user must enter his login credential (username and password) on ARDI web app in order to achieve the submission.
